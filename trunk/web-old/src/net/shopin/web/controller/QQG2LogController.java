...
public class QQG2LogController {
    ...
    private static String QQ_FANLI_CONNECT_KEY1 = LoadProperties.readValue("QQ_FANLI_CONNECT_KEY1");
    private static String QQ_FANLI_CONNECT_KEY2 = LoadProperties.readValue("QQ_FANLI_CONNECT_KEY2");
    private static  final String MALL_HOME_PAGE_URL = "http://www.shopin.net";

    private static String QQ_API_SECRET = LoadProperties.readValue("QQ_API_SECRET");
    private static String QQ_GETADDRESS = LoadProperties.readValue("QQ_GETADDRESS");
    ...
    @RequestMapping(value = "/qqG2Login", method = {RequestMethod.GET, RequestMethod.POST})
    public String retrun(Model m, HttpServletRequest request, HttpServletResponse response) {
       ...
       //统计字段，下单后需要回传
        ...
        if (memberVO != null) {
            ...
            String uid2 = UUID.randomUUID().toString();
             if( viewinfoMap.get("HeadShow")!=null&& viewinfoMap.get("HeadShow").toString().length()>0){
               ...
               // System.out.print("HeadShow:"+HeadShow+"JifenUrl:"+JifenUrl+"OpenKey:"+OpenKey+"Ts:"+Ts+"OpenId:"+OpenId);
                saveCookie(response,"CB_log","CB_log");
                 List<MemberAddressVO> addresslist=this.getCbAddress(OpenId,OpenKey,Ts) ;
                 if(addresslist!=null&&addresslist.size()>0){
                     try{
                            memberService.sysChroAddressFromCb(memberVO,addresslist);
                     } catch (Exception e){
    ...
    private String getLowerMd5(String md5str) {
     ...
     public List<MemberAddressVO> getCbAddress(String openId,String openKey,String Ts){
                String s=this.getLowerMd5(CHARSET+MerchantId+openId+openKey+ReturnFmt+Ts+Version+QQ_API_SECRET)  ;
                HttpClient client = new HttpClient();
		    	client.setConnectionTimeout(3000);//设置为延时3秒
				client.setTimeout(3000);
                String  url=QQ_GETADDRESS+"?version="+Version+"&merchant_id="+MerchantId+"&openid="+openId+"&openkey="+openKey+"&charset="+CHARSET+"&return_fmt="+ReturnFmt+"&timestamp="+Ts+"&sign="+s;
			    HttpMethod method=new GetMethod(url);
				try {
                    client.executeMethod(method);
                    String j= method.getResponseBodyAsString();
                    List<MemberAddressVO> addresslist=new ArrayList();
                    JSONObject json = JSONObject.fromObject(j);
                     if(json.get("ret_msg").equals("ok")){
                        JSONArray shopping_address=(JSONArray) json.get("shopping_address");
                        for(int i=0;i<shopping_address.size();i++){
                            JSONObject shopping_add= shopping_address.getJSONObject(i);
                            String province=shopping_add.get("province")!=null?URLDecoder.decode(shopping_add.get("province").toString(), "UTF-8"):"";
                            String city=shopping_add.get("city")!=null?URLDecoder.decode(shopping_add.get("city").toString(), "UTF-8"):"";
                            String dist=shopping_add.get("dist")!=null?URLDecoder.decode(shopping_add.get("dist").toString(), "UTF-8"):"";
                            String address=shopping_add.get("address")!=null?URLDecoder.decode(shopping_add.get("address").toString(), "UTF-8"):"";
                            String post_code=shopping_add.get("post_code")!=null?shopping_add.get("post_code").toString():"";
                            String mobile=shopping_add.get("mobile")!=null?shopping_add.get("mobile").toString():"";
                            String name=shopping_add.get("name")!=null?URLDecoder.decode(shopping_add.get("name").toString(), "UTF-8"):"";
                            String phone=shopping_add.get("phone")!=null?shopping_add.get("phone").toString():"";
                            MemberAddressVO maddress=new MemberAddressVO();
                            maddress.setProvinceName(province);
                            maddress.setCityName(city);
                            maddress.setAddress(address);
                            maddress.setPostcode(post_code);
                            maddress.setMobileTel(mobile);
