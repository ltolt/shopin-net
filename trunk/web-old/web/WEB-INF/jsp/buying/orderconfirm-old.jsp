...
function getAddressData() {
    var address_radios = $(":radio[name='cart_address_item'][checked]");
...
<%--选择地址列表--%>
function selectAddressOption(radio) {
    if ($(radio).val() == '') {
...
function renderPayTypeOptions(existCollect) {
    ...
    $(":radio[name='cart_paytype_item']").each(function() {
        ...
        var paytypename = radio.next().html();
        if ('货到付款' == paytypename) {
...
function renderPaytypeInfo() {
    ...
    $("#cart_paytype").html("<li>支付方式：" + paytype + "</li>");
}
...
function commitPaytypeForm(button) {
    ...
    var paytypename = input.next().html();
    $.ajax({
...
<div class="pays-tcony" id="cart_address_title">
    <div class="pay-tcon-re">收货人信息</div>
...
<div class="pays-tinfo" id="cart_address_select"
    ...
    <li>
                    <span>
                        <input type="radio" name="cart_address_item" value="" onClick="selectAddressOption(this)"/>
                        <label>使用新地址</label>
                    </span>
    </li>
</div>
...
<div class="pays-tinfo" id="cart_paytype_form"
    ...
    <c:forEach items="${paymentlist}" var="item">
        <c:if test="${item.sid=='10'}">
            <li>
                <input type="radio" name="cart_paytype_item" value="${item.sid}" checked='true'/>
   ...
   <%} else if (cid.equals("shopin-cmb")) {%>
    ...
    <%} else {%>
    <c:forEach items="${paymentlist}" var="item">
        <li <c:if test="${!existCollect&&item.name=='货到付款'}">style='display:none'</c:if> >
            <input type="radio" name="cart_paytype_item" value="${item.sid}"
