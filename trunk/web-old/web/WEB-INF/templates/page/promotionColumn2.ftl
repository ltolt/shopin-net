<#--
    Description:活动-限时抢购-栏目模板(抢购活动-展示内容:展示3条信息)
    Author: Peter Wei
    Date: 2010-2-5
-->
                ...
                <#list root.productList as product>
                    <li>
                        <a href="${ctx}/product/${product.sid}.html">
                            <div><img src="${images}/del/d-16.gif" alt="产品图片"/></div>
                            <p class="height36">${product.productName?if_exists}</p>

                            <div class="remainSeconds"
                                 style="display:none">${product.proPrice?if_exists.remainSeconds?if_exists}</div>
