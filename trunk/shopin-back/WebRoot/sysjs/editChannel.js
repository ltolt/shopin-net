//增加栏目商品窗口扩展
EditChannelWindow = Ext.extend(Ext.Window,{
	  gridpanel   : null,
	  selectpanel : null,
	  resultpanel : null,
	  json : null,
	  constructor : function(a){
		  if(a == null) {
			...
			Ext.apply(this, a);
			this.initComponents();
			EditChannelWindow.superclass.constructor.call(this,{
	...
	//	store.load();
		...
		// var str = {'success':'true','result':[]};
		  var rstore = new  Ext.data.JsonStore({
			url : __ctxPath + "/roles/selectOthers",
			root : "list",
			baseParams : {
		  ...
		  this.gridpanel = new Ext.Panel({
			  ...
			  bbar:new Ext.Toolbar({
					...
					items:[
					{
						...
						text:'取消',
						handler:function(){
							Ext.getCmp("EditChannelWindow").close();
						}
					}]
			  }),
			  items:[
