AddPhotoDetailForm=Ext.extend(Ext.Window ,{
			    		...
			    		items:[
{
				...
				bbar:new Ext.Toolbar({
					...
					items:[
					{
							...
							iconCls: 'button_save',
							allowDepress:true,
							 handler: this.savedetail.createCallback(mapstore,detaildata)
						},
					{
						xtype:'button',
						preesed:true,
						...
						text:'取消',
						handler:function(){
							Ext.getCmp('AddPhotoDetailWin').close();
...
savedetail : function(mapstore,detaildata){
...
//	var priceUpdateTime = Ext.getCmp('detailpanel').form.findField("priceUpdateTime").getValue();
	var priceUpdateTime = Ext.get("priceUpdateTime").dom.value
	mapstore.each(function(r){
		...
		// var detaildata =   Array();
		 detaildata.push(Ext.decode(json));
	})
	
	Ext.getCmp('detailgrid').getStore().loadData(detaildata);
