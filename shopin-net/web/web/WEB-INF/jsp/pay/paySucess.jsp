...
<div id="container">
		<div class="in oz">
				<div class="cart-step steps6 mt"></div>
				<div class="buy-success mt mb pr clear">
				<div class="ml78">
				<h2 class="mt20 pr"><i class="ico-success"></i>感谢您，订单已提交成功！</h2>
				<p>您的订单号：${order.orderNo }<u>|</u>${order.paymentModeName }：<span class="txt-red f20">￥${order.needSaleMoneySum }</span> <u>|</u>${order.sendTypeName }&nbsp;&nbsp;&nbsp;${order.memo }</p>
				<p class="txt-edit"><a href="${sso}/myshopin/vieworders.html?orderNo=${order.orderNo }&status=view">查看订单 >></a></p>
				...
				<p class="mb">常规情况下，您的订单提交并付款后，我们会在上品商业旗下多家实体店进行货品调拨，所有货品统一调拨至物流中心后再配送至您指定的地点。（调拨时间通常为1-3天后开始配送发货）</p>
					<div class="mt pro-list"><h4>购买此商品的用户还买了：</h4>
						<c:forEach items="${webProductVOList }" var="item">
							<ul>
								<li><a href="../product/${item.id}.html"><img src="${ftpimages}${item.productImages }" width="130" height="88"></a></li>
								<li><a href="../product/${item.id}.html">${item.productName }</a></li>
								<li><span class="price">¥${item.proCurPrice}</span><span class="discount">(${item.offValue}折)</span></li>
							</ul>
						</c:forEach>
					</div>
				</div>
